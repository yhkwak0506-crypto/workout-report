...
st.set_page_config(page_title="S-Tier Performance AMS v9.0", layout="wide")
...
st.title("⚡ S-Tier AI Coach System (v9.0)")
if not HAS_AI:
    st.warning("⚠️ 제미나이 AI API 키가 설정되지 않았습니다. 일부 자동 분석 기능이 제한됩니다.")
...
with tab_morning:
    ...
    calc_sleep_mins = max(0, (total_bed_mins - 20) * 0.9) # 20분 잠복기, 90% 효율
...
with tab_workout:
    ...
    if workout_type == "개인 축구 훈련":
        ...
        with c2: c_dist = st.text_input("거리/개수", "5km")
...
# ------------------------------------------
# 🥗 TAB 3: AI 식단 관리 (칼로리 입력 제거, AI 자동 연산)
# ------------------------------------------
with tab_diet:
    ...
    with col_d1:
        breakfast = st.text_area("🌅 아침 식단", height=80, placeholder="예: 닭가슴살 150g, 오트밀 80g")
        lunch = st.text_area("☀️ 점심 식단", height=80, placeholder="예: 소고기 우둔살 200g, 현미밥 한공기")
        night = st.text_area("🌌 야식", height=80, placeholder="예: 무지방 그릭요거트")
    with col_d2:
        dinner = st.text_area("🌙 저녁 식단", height=80, placeholder="예: 연어 구이 200g, 샐러드")
        snacks = st.text_area("🥤 간식/보충제", height=80, placeholder="예: 신타6 1스쿱, 아몬드 10알")
    ...
    if st.button("🤖 AI 칼로리 분석 및 구글 시트 저장"):
        if HAS_AI:
            ...
            st.error("AI API가 연결되지 않아 직접 시트에 텍스트만 저장합니다.")
...
with tab_report:
    ...
    if st.button("🤖 AI 개인 코치 호출하기 (내일 훈련 설계)"):
        if HAS_AI:
            ...
            st.error("⚠️ AI 코치를 호출하려면 먼저 Gemini API 키를 Secrets에 입력해야 합니다!")
