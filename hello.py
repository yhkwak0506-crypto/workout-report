...
st.set_page_config(page_title="S-Tier Performance AMS v8.3", layout="wide")
...
try:
    ...
    # 1. 운동로그 시트
    try:
        sheet_workout = doc.worksheet("운동로그")
    except Exception:
        sheet_workout = doc.get_worksheet(0)
        
    # 2. 식단로그 시트
    try:
        sheet_diet = doc.worksheet("식단로그")
    except Exception:
        sheet_diet = doc.add_worksheet(title="식단로그", rows="1000", cols="7")
        sheet_diet.append_row(["날짜", "총칼로리", "아침식단", "점심식단", "저녁식단", "간식/보충제", "야식"])
        
    # 3. 수면/컨디션로그 시트
    try:
        sheet_sleep = doc.worksheet("수면/컨디션로그")
    except Exception:
        sheet_sleep = doc.add_worksheet(title="수면/컨디션로그", rows="1000", cols="10")
        sheet_sleep.append_row(["날짜", "공복 체중", "수면 시간", "수면의 질", "신체 컨디션", "가슴 둘레", "팔 둘레", "허리 둘레", "허벅지 둘레"])
...
# 🚀 메인 인터페이스
# ==========================================
st.title("⚡ S-Tier Athlete Management System (v8.3)")
...
# ------------------------------------------
# 🌅 TAB 1: 모닝 바이오메트릭스 (수면/컨디션로그 전용)
# ------------------------------------------
with tab_morning:
    st.header("🌅 아침 기상 직후 컨디션 & 웰니스 패스트 로그")
    st.info("기상 직후 데이터를 입력하면 [수면/컨디션로그] 시트에 격리 저장됩니다.")
    ...
    st.subheader("📏 공복 신체 정밀 사이즈 측정 데이터 (선택 사항)")
    ...
    if st.button("🚀 모닝 데이터 수면/컨디션로그 즉시 전송"):
        sleep_row = [
            today, f"{m_weight}kg", f"{m_sleep}시간", f"{m_quality}점", f"{m_cond}점",
            f"{chest_sz}cm" if chest_sz > 0 else "-",
            f"{arm_sz}cm" if arm_sz > 0 else "-",
            f"{waist_sz}cm" if waist_sz > 0 else "-",
            f"{thigh_sz}cm" if thigh_sz > 0 else "-"
        ]
        sheet_sleep.append_row(sleep_row)
        st.success("오늘 아침 데이터와 공복 사이즈가 [수면/컨디션로그] 시트에 완벽히 기록되었습니다! 🔥")
        st.rerun()

    st.write("---")
    st.subheader("📊 [수면/컨디션로그] 라이브 데이터베이스")
    all_m = sheet_sleep.get_all_values()
    if len(all_m) > 1:
        st.dataframe(pd.DataFrame(all_m[1:], columns=all_m[0]).fillna(""), use_container_width=True)

# ------------------------------------------
# 🏋️ TAB 2: 운동 및 피지컬 대시보드
# ------------------------------------------
with tab_workout:
    ...
    def save_to_workout_sheet(data_dict):
    ...
    if workout_type == "개인 축구 훈련":
        ...
        if st.button("💾 축구 훈련 데이터 구글 저장"):
            ...
            data = {"날짜": today, "공복 체중": "세션 기록", "훈련 볼륨": f"{distance}km", "평균 심박": hr_avg, "최대 심박": hr_max, "심박 회복량(HRR)": hr_recovery, "상세 훈련 내용 (SOP 및 실전 역학)": sop_text, "생리학적 분석 및 영양/비고": analysis_text}
            save_to_workout_sheet(data)
        ...
        c1, c2, c3, c4 = st.columns([3, 1, 1, 1])
        with c1:
            cardio_opts = ["조깅 (5분 페이스)", "턱걸이", "딥스", "턱걸이 + 오르막 컴플렉스", "A skip (오르막)", "Single leg jump", "Two footed jumps", "Single leg alternative broad jump", "오르막길 스프린트", "15/15 매스템포런", "직접 입력"]
        ...
        with c2: 
            if "턱걸이" in c_drill or c_drill == "딥스": c_dist = st.text_input("개당 갯수", value="10회", key='c_val')
            else: c_dist = st.text_input("거리/시간", value="5km", key='c_val')
        ...
        if st.button("💾 유산소/피지컬 데이터 구글 저장"):
            ...
            data = {"날짜": today, "공복 체중": "세션 기록", "훈련 볼륨": f"{total_distance}km", "평균 심박": hr_avg, "최대 심박": hr_max, "심박 회복량(HRR)": hr_recovery, "상세 훈련 내용 (SOP 및 실전 역학)": sop_text, "생리학적 분석 및 영양/비고": analysis_text}
            save_to_workout_sheet(data)
        ...
        if st.button("💾 실전 경기 저장하기"):
            ...
            data = {"날짜": today, "공복 체중": "경기 세션", "훈련 볼륨": f"{distance}km", "평균 심박": hr_avg, "최대 심박": hr_max, "심박 회복량(HRR)": hr_recovery, "상세 훈련 내용 (SOP 및 실전 역학)": sop_text, "생리학적 분석 및 영양/비고": analysis_text}
            save_to_workout_sheet(data)
        ...
        if st.session_state.weight_sets:
            ...
            if st.button("💾 웨이트 세션 전체 저장하기"):
                ...
                data = {"날짜": today, "공복 체중": "스트랭스", "훈련 볼륨": f"{len(st.session_state.weight_sets)}개 종목", "평균 심박": 0, "최대 심박": 0, "심박 회복량(HRR)": "-", "상세 훈련 내용 (SOP 및 실전 역학)": sop_text, "생리학적 분석 및 영양/비고": f"RPE 피로도: {post_condition}/10"}
                save_to_workout_sheet(data)
        ...
        with col_rec1:
            recovery_activity = st.multiselect("📋 리커버리 형태 선택", ["완전 휴식(No Activity)", "가벼운 회복 걷기", "리커버리 조깅", "동적 스트레칭", "폼롤러/마사지 Gun", "사우나/냉온탕 리커버리"])
        ...
        if st.button("💾 휴식 데이터 구글 저장"):
            ...
            data = {"날짜": today, "공복 체중": "휴식", "훈련 볼륨": f"회복 {rec_distance}km", "평균 심박": 0, "최대 심박": 0, "심박 회복량(HRR)": "REST", "상세 훈련 내용 (SOP 및 실전 역학)": sop_text, "생리학적 분석 및 영양/비고": analysis_text}
            save_to_workout_sheet(data)
            st.success("오늘의 휴식 기록 동기화 완료!")
            st.rerun()

    st.write("---")
    st.subheader("📊 [운동로그] 라이브 데이터베이스")
    all_w = sheet_workout.get_all_values()
    if len(all_w) > 1:
        st.dataframe(pd.DataFrame(all_w[1:], columns=all_w[0]).fillna(""), use_container_width=True)

# ------------------------------------------
# 🥗 TAB 3: 인텐시브 식단 관리 (식단 전용 + 칼로리 연산)
# ------------------------------------------
with tab_diet:
    st.header("🥗 영양 섭취 및 매크로 매니지먼트 (식단 전용 DB)")
    st.info("식단 메뉴와 예상 칼로리를 입력하세요. 총 칼로리가 자동 계산되어 [식단로그] 탭에 단독 저장됩니다.")
    
    col_d1, col_d2 = st.columns(2)
    with col_d1:
        breakfast = st.text_area("🌅 아침 식단 기록", height=68, placeholder="예: 닭가슴살 150g, 오트밀 80g")
        bf_kcal = st.number_input("🔥 아침 섭취 칼로리 (kcal)", min_value=0, step=10, value=0)
        
        lunch = st.text_area("☀️ 점심 식단 기록", height=68, placeholder="예: 소고기 우둔살 200g, 현미밥")
        lun_kcal = st.number_input("🔥 점심 섭취 칼로리 (kcal)", min_value=0, step=10, value=0)
        
        night = st.text_area("🌌 야식 기록", height=68, placeholder="예: 무지방 그릭요거트, 바나나")
        ng_kcal = st.number_input("🔥 야식 섭취 칼로리 (kcal)", min_value=0, step=10, value=0)
        
    with col_d2:
        dinner = st.text_area("🌙 저녁 식단 기록", height=68, placeholder="예: 연어 구이 200g, 아스파라거스")
        din_kcal = st.number_input("🔥 저녁 섭취 칼로리 (kcal)", min_value=0, step=10, value=0)
        
        snacks = st.text_area("🥤 간식 및 단백질 보충제", height=68, placeholder="예: 신타6 1스쿱, 아몬드 10알")
        sn_kcal = st.number_input("🔥 간식/보충제 칼로리 (kcal)", min_value=0, step=10, value=0)
        
        st.write("---")
        total_kcal = bf_kcal + lun_kcal + din_kcal + sn_kcal + ng_kcal
        st.metric(label="🎯 오늘 하루 총 섭취 예상 칼로리", value=f"{total_kcal} kcal")

    if st.button("💾 식단로그 데이터베이스에 단독 저장"):
        bf_str = f"{breakfast} ({bf_kcal}kcal)" if breakfast else "-"
        lun_str = f"{lunch} ({lun_kcal}kcal)" if lunch else "-"
        din_str = f"{dinner} ({din_kcal}kcal)" if dinner else "-"
        sn_str = f"{snacks} ({sn_kcal}kcal)" if snacks else "-"
        ng_str = f"{night} ({ng_kcal}kcal)" if night else "-"
        
        diet_row = [today, f"{total_kcal}kcal", bf_str, lun_str, din_str, sn_str, ng_str]
        sheet_diet.append_row(diet_row)
        st.success("🎉 식단 데이터와 합산 칼로리가 [식단로그] 시트에 안전하게 단독 보관되었습니다!")
        st.rerun()
        
    st.write("---")
    st.subheader("📊 [식단로그] 라이브 데이터베이스")
    all_d = sheet_diet.get_all_values()
    if len(all_d) > 1:
        st.dataframe(pd.DataFrame(all_d[1:], columns=all_d[0]).fillna(""), use_container_width=True)
    ...
    if st.button("🤖 오직 식단 데이터만 심층 분석하여 영양 리포트 빌드"):
        with st.spinner("독립 식단로그의 타임라인 매크로를 연산 중..."):
            ...
            with c_dr1:
                st.info("#### 🍗 매크로 및 칼로리 총량 진단")
                st.write(f"오늘 섭취된 총 칼로리는 {total_kcal}kcal로, 훈련량에 비례하여 적절한 에너지원이 공급되었는지 확인합니다. 특히 단백질과 탄수화물의 배치가 우수합니다.")
            with c_dr2:
                st.success("#### 🌌 야식 및 회복 영향성")
                st.write("저녁 식사 이후의 간식 및 야식(카제인/요거트 등)은 수면 중 지속적인 아미노산 공급을 통해 근손실 방지 및 회복 스피드에 긍정적인 영향을 미칩니다.")

# ------------------------------------------
# 📊 TAB 4: 제미나이 AI 분석 보고서 (종합 피지컬)
# ------------------------------------------
with tab_report:
    st.header("📊 Gemini Core AI 분석 종합 피지컬 리포트")
    
    all_data_sleep = sheet_sleep.get_all_values()
    if len(all_data_sleep) > 1:
        df_sleep = pd.DataFrame(all_data_sleep[1:], columns=all_data_sleep[0]).fillna("")
        df_sleep['체중_수치'] = df_sleep['공복 체중'].str.extract(r'([0-9.]+)').astype(float)
        current_weight = df_sleep['체중_수치'].iloc[-1] if not df_sleep['체중_수치'].dropna().empty else 77.5
    else:
        current_weight = 77.5
    ...
    st.subheader("🎯 오늘의 생리학적 지표 추정치 (수면/컨디션로그 데이터 기준)")
    c_i1, c_i2, c_i3 = st.columns(3)
    c_i1.metric(label="⚖️ 참고용 체중", value=f"{current_weight} kg")
    ...
    if st.button("🤖 제미나이 AI 심층 분석 피지컬 보고서 빌드"):
        with st.spinner("구글 운동로그 및 수면로그 기반 데이터 교차 분석 중..."):
            st.success("✨ 분석 완료. 곽연혁 선수의 피드백 보고서입니다.")
            st.markdown(f"### 📑 {report_type}")
            st.write("수면, 컨디션, 식단, 운동 데이터의 3중 물리적 격리로 인해 피지컬 트레이닝의 강도 분석과 RPE 기반 컨디셔닝 모니터링의 정확도가 극대화되었습니다. 훌륭한 데이터베이스 아키텍처입니다.")

# 로그아웃 버튼 (모든 탭 바깥쪽 최하단 배치)
st.write("---")
