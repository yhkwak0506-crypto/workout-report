...
def save_single_meal(col_idx, text_val):
    ...
    for idx, r in enumerate(all_d_current):
        if r[0] == today:
            ...
            break
    if row_idx:
        sheet_diet.update_cell(row_idx, col_idx, text_val)
    else:
        new_row = [today, "0kcal", "", "", "", "", ""]
...
# ------------------------------------------
# 📊 TAB 1: 신체 데이터 (운동 데이터의 체중 연동 자동화)
# ------------------------------------------
with tab_body:
    ...
    # 💡 [V10.4 업데이트] 운동 로그에서 공복체중을 동적으로 연동하여 그래프 작성
    st.write("---")
    st.subheader("📈 엘리트 바디 컴포지션 트렌드 (운동 탭 체중 데이터 연동)")
    
    all_w_for_body = get_cached_data("workout")
    if len(all_w_for_body) > 1:
        df_body = pd.DataFrame(all_w_for_body[1:], columns=all_w_for_body[0])
        ...
        if len(df_body) >= 1:
            ...
            chart_data = df_body.sort_values('날짜').set_index('날짜')[['추정 골격근량(kg)', '추정 체지방량(kg)']]
            st.line_chart(chart_data, color=["#1f77b4", "#ff7f0e"])
        else:
            st.info("체중 그래프를 활성화하려면 운동 데이터 탭에서 공복 체중을 포함하여 최소 1회 이상 저장해 주세요.")
    else:
        st.info("운동 데이터베이스가 비어 있습니다.")

# ------------------------------------------
# 🏋️ TAB 2: 운동 데이터 (공복체중 마스터 필드 도입)
# ------------------------------------------
with tab_workout:
    st.header("🧠 오늘의 트레이닝 세션 로깅")
    
    # 💡 모든 탭의 기준이 되는 공복체중 마스터 입력창
...
with tab_diet:
    ...
    # 💡 [V10.4 업데이트] UI 옆에 실시간 매칭 칼로리 및 단독 저장 기능 배치
    meals_config = [
    ...
    if st.button("🧠 현재까지의 영양소 크로스-매칭 및 다음 식사 추천"):
        if HAS_AI:
            with st.spinner("분석 중..."):
                ...
                [오늘의 운동량]: {today_w_str}
                [현재까지 식단 기록]: 아침({raw_b}), 점심({raw_l}), 저녁({raw_d}), 간식({raw_s}), 야식({raw_n})
                위의 데이터를 기반으로 다음 식사에서 보충해야 할 탄수화물/단백질 비율과 맞춤 메뉴를 현실적인 칼로리로 쪼개서 추천해줘. 호들갑 경고 문구 금지.
                """
                st.markdown(ask_gemini(prompt))

# ------------------------------------------
# 📈 TAB 4: 데이터/리포팅 센터 (💡 당일 실시간 분할 분석 보고서 신설)
# ------------------------------------------
with tab_report:
    ...
    st.write("---")
    # 💡 [V10.4 업데이트] '실시간 당일 분석 보고서 (Real-time Live)' 추가
            ...
            with st.spinner("데이터 동기화 및 훈련 역학 스캔 중..."):
                try:
                    ...
                    if "Real-time" in report_type:
                        # 💡 오늘 입력된 데이터만 완전 추출해서 실시간 분할 보고서 생성
                        ...
                        s_ctx = f"수면:{today_s_data[0][2]}, 기상컨디션:{today_s_data[0][4]}" if today_s_data else "오늘 수면 데이터 미등록"
