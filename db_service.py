...
# 💡 V15.1 업데이트: 식단 탭에 수동 칼로리(Column B) 로깅 기능 추가
...
def render_diet_tab(today: str, bootcamp_mode: bool):
    ...
    # 💡 Column B(칼로리)부터 Column G(야식)까지 인덱스 매핑: 2=칼로리, 3=아침, 4=점심...
    ...
    if bootcamp_mode:
        ...
        # 💡 [새 기능] 상단에 총 칼로리를 직접 입력할 수 있는 필드 추가
        st.subheader("📊 오늘의 총 섭취 칼로리")
        c_cal1, c_cal2 = st.columns([4, 1])
        with c_cal1:
            current_cal = utils.extract_number(raws[2]) if raws[2] else 0.0
            daily_cal = st.number_input("🔥 칼로리 직접 입력 (kcal)", min_value=0, step=50, value=int(current_cal), key="daily_cal_input")
        with c_cal2:
            st.write("")
            st.write("")
