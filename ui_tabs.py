...
def render_body_tab(today: str):
    ...
    m_weight = st.number_input("⚖️ 오늘의 공복 체중 (kg)", min_value=0.0, value=st.session_state.get("master_weight", 77.5), step=0.1)
    ...
    if len(all_s_body) > 1:
        ...
        if not df_s.empty:
            ...
            if st.button("🤖 AI 북유럽 리그 기준 피지컬 진단"):
...
def render_workout_tab(today: str, bootcamp_mode: bool):
    ...
    if bootcamp_mode:
        ...
        if bc_time == "메인 일과":
            ...
            bc_comment = st.text_area("✍️ 체감 코멘트 (예: 행군 발바닥 물집, 엄청 지침)")
            if st.button("💾 저장"):
            ...
            bc_comment = st.text_area("✍️ 체감 코멘트")
        ...
        if workout_type == "개인 축구 훈련":
            ...
            dist_val = st.number_input("🏃 거리(km)", 0.0, step=0.1)
            details_str = f"[{time_of_day}] {location} | {' ➡️ '.join(st.session_state.football_drills)}"

        elif workout_type == "유산소/조깅":
            c_drill = st.selectbox("📋 종목", ["러닝", "턱걸이", "딥스", "오르막 스프린트", "직접 입력(Custom)"])
            if c_drill == "직접 입력(Custom)": c_drill = st.text_input("📝 종목 직접 입력")
            if c_drill == "러닝":
            ...
            dist_val = st.number_input("🏃 총 거리(km)", 0.0, step=0.1)
            ...
            is_not_sure = st.checkbox("🤔 기기 장착 안 함 (기록 측정 불가)", value=False)
            dist_val = 0.0 if is_not_sure else st.number_input("거리(km)", 0.0, step=0.1)
            details_str = f"[{time_of_day}] {match_type}"

        elif workout_type == "웨이트 트레이닝":
            is_super = st.radio("💪 세트 방식", ["단일 종목", "슈퍼/컴파운드 세트"], horizontal=True)
            if is_super == "단일 종목":
                c1, c2, c3, c4 = st.columns(4)
                with c1: w_ex = st.text_input("운동명")
                with c2: w_kg = st.number_input("무게(kg)", step=2.5)
                with c3: w_rep = st.number_input("횟수", step=1)
                with c4: w_set = st.number_input("세트", step=1)
                if st.button("➕ 추가"): st.session_state.weight_sets.append(f"{w_ex}({w_kg}kg x{w_rep}회 {w_set}세트)")
            else:
                c1, c2, c3 = st.columns(3)
                with c1: s_ex1 = st.text_input("종목 1 (예: 벤치 60kg)")
                with c2: s_ex2 = st.text_input("종목 2 (예: 풀업)")
                with c3: s_set = st.number_input("총 세트 수", step=1)
                if st.button("➕ 슈퍼세트 추가"): st.session_state.weight_sets.append(f"슈퍼[{s_ex1}+{s_ex2}] x{s_set}세트")
            
            if st.session_state.weight_sets:
                st.info("👉 " + " / ".join(st.session_state.weight_sets))
                if st.button("🗑️ 지우기"): st.session_state.weight_sets = []; st.rerun()
            details_str = f"[{time_of_day}] " + " / ".join(st.session_state.weight_sets)

        elif workout_type == "휴식":
            rec_act = st.text_input("활동 (예: 폼롤러 20분)")
        ...
        if is_not_sure: h_avg, h_max, hrr_2m = 0, 0, "-"
            ...
            with col_h1: h_avg = st.number_input("❤️ 평균 심박 (bpm)", min_value=0, step=1)
            with col_h2: h_max = st.number_input("🔥 최대 심박 (bpm)", min_value=0, step=1)
            with col_h3: hrr_2m = st.text_input("📉 2분 심박 회복량 (HRR)")
            
        user_comment = st.text_area("✍️ 주관적 체감 코멘트 (AI 학습용)")
        if st.button("💾 최종 운동 저장"):
...
def render_diet_tab(today: str, bootcamp_mode: bool):
    ...
    if bootcamp_mode:
        ...
        for name, idx in zip(["아침", "점심", "저녁", "간식", "야식"], [3,4,5,6,7]):
            ...
            with c1: input_val = st.text_area(name, value=raws[idx], height=68, key=f"d_{idx}")
    ...
    if st.button("🤖 S-Tier 분석 리포트 발행"):
        with st.spinner("AI 딥러닝 스캔 중..."):
            try:
                ...
                prompt = f"""
                수석 피지컬 코치야. 목표: 2027년 말 북유럽, 아일랜드 1~2부 리그 진출.
